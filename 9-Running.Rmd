...
Despite the scary warnings in the next pop up, you can add and
...
survey and checked its data file.
Check that **Date stamp** is On, so you can work out
when each participant did the survey. Leave the rest Off.
You do want **Open-access mode** unless you are using a
...
| Question Code: | ![ ](media/image84.png) |


| Abbreviated question text: | ![ ](media/image85.png) |


| Full question text: | ![ ](media/image86.png) |


| Question code and question text: | ![ ](media/image87.png) |
...
mtf<-data %>% select(-participant, starts_with("MTF")) %>% # select ID and MTF vars
  ...
  mutate(rating=case_when( # create a new variable rating
      ...
      rating = ifelse(name %in% c("MTF1", "MTF3", "MTF9"), # if the name is in this list
