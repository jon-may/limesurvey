...
```         
data<-read.csv("results-survey531857-5.csv") # read the limesurvey data

var.item<-tibble(cols=colnames(data)) %>% # make a tibble from data's columnames
           mutate(cols=str_replace(cols,"\\.\\.","_"),      # replace .. with _
                  var=str_extract(cols, "[A-Za-z0-9\\.]*"), # select all text up to _
                  item=str_sub(cols, str_length(var)+1,-1), # select everything after the _
                  item=str_replace_all(item, "_", "")       # delete the _
                  )
...
## Printing a copy of your survey

You may need to create a PDF of your survey to attach to an Ethics
application or to include as an appendix in a report. This is easy to do
within the survey.

To get a printable copy of the survey, click the Export button at the
top of the screen :

![](media/image96.png)

Then select Printable from the options available. This will let you
print or save a copy of your survey (print to PDF).
